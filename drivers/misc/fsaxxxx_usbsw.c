...
#if defined(CONFIG_MACH_KYLE_I) || defined(CONFIG_MACH_KYLE_CHN) /* since certain dock couldn't be recognized as a charger */
u8 intr2_av  = 0;
#endif
...
#if defined(CONFIG_MACH_KYLE_I) || defined(CONFIG_MACH_KYLE_CHN)
		if (intr2_av & (1 << 0)) {
			if (val2 & (1 << 6)) {
				if (pdata->charger_cb)
					pdata->charger_cb(FSA_ATTACHED, &ops);
